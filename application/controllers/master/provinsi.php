...
class provinsi extends REST_Controller {
    ...
    // show data siswa
    function index_get() {
        // $kd_prov = $this->get('kd_prov');
        $kd_prov = $this->uri->segment(3);
        ...
        if (Authorization::tokenIsExist($headers)) {
            ...
            if ($token != false) {    
        if ($kd_prov == '') {
            $m_provinsi = $this->db->get('m_provinsi')->result();
        } else {
            $this->db->where('kd_prov', $kd_prov);
            $m_provinsi = $this->db->get('m_provinsi')->row();
        }
        $this->response($m_provinsi, 200);
        }
            else {
                $response = [
    ...
    function index_post() {
        ...
        if (Authorization::tokenIsExist($headers)) {
            $token = Authorization::validateToken($headers['Authorization']);
            if ($token != false) {    
        $insert = $this->db->insert('m_provinsi', $data);
        if ($insert) {
    ...
    function index_put() {
        ...
        if (Authorization::tokenIsExist($headers)) {
            ...
            if ($token != false) {    
        $this->db->where('kd_prov', $kd_prov);
        $update = $this->db->update('m_provinsi', $data);
    ...
    function index_delete($kd_prov) {
        ...
        if (Authorization::tokenIsExist($headers)) {
            ...
            if ($token != false) {    
        $this->db->where('kd_prov', $kd_prov);
        $delete = $this->db->delete('m_provinsi');
        // ---
